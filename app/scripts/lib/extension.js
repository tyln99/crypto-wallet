...
import { getEnvironmentType, checkForError } from './utils'
import { ENV_TYPE_BACKGROUND, TRANSACTION_STATUSES } from '../../../shared/constant/app'
...
export default class ExtensionPlatform {
  ...
  // add name to remove listener with method
  closeCurrentWindow() {
  ...
  getVersion() {
    const { version, version_name: versionName } = extension.runtime.getManifest()
  ...
  openExtensionInBrowser(route = null, queryString = null, keepWindowOpen = false) {
    let extensionURL = extension.runtime.getURL('home.html') //check is support fullscreen
    ...
    if (getEnvironmentType() !== ENV_TYPE_BACKGROUND && !keepWindowOpen) {
      window.close()
    }
  }
  ...
  addOnRemovedListener(listener) {
    try {
      extension.windows && extension.windows.onRemoved.addListener(listener)
    } catch (error) {
      // logErr(error)
    }
  }
